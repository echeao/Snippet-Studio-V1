 ...
 * 2. 依赖倒置 (DIP): 暴露抽象接口 [ISnippetRepository] 与 [ISettingsRepository]，便于单测与解耦。
 * 3. 惰性单例 (by lazy): 采用 Kotlin 默认的线程安全双重检查锁 (Synchronized LazyThreadSafetyMode.SYNCHRONIZED)，
...
class AppContainer(private val context: Context) {
    ...
    /**
     * 代码片段业务仓库契约接口 (Snippet Repository Contract)。
     * 依赖组合：注入 `database.snippetDao()` 与 `database.folderDao()` 数据访问对象、`context` 上下文及 `gitManager` 底层组件。
     */
    val snippetRepository: ISnippetRepository by lazy {
        SnippetRepository(
    ...
    /**
     * 全局设置偏好仓库契约接口 (Settings Repository Contract)。
